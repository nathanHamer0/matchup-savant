...
def load_batter(name: str) -> dict[str | int, DataFrame]:
    ...
    for p in PITCH_TYPES:
        pitch_type = pandas.read_csv(
            os.path.join(BATTER_PITCH_TYPE_DIR, f"batter_{p}.csv")
        )
        batter_pitch_type = pitch_type[
            (pitch_type["player_name"]).str.lower() == name.lower()
        ]  # Standardize to lower-case in case of 'McDonald's' type captialization obscuring equivalencies
...
def load_pitcher(name: str) -> dict[str | int, DataFrame] | int:
    ...
    for p in PITCH_TYPES:
        pitch_type = pandas.read_csv(
            os.path.join(PITCHER_PITCH_TYPE_DIR, f"pitcher_{p}.csv")
        )
        pitcher_pitch_type = pitch_type[
            (pitch_type["player_name"]).str.lower() == name.lower()
        ]
...
def get_all_batters() -> list[str]:
    ...
    for b in batters_df["player_name"]:
        batters.append(
            {
                "standard_name": name_utils.inverted_name_to_standard_name(b),
                "snake": name_utils.inverted_name_to_snake(b),
            }
        )
...
def get_all_pitchers() -> list[dict[str, str]]:
    ...
    for p in pitchers_df["player_name"]:
        pitchers.append(
            {
                "standard_name": name_utils.inverted_name_to_standard_name(p),
                "snake": name_utils.inverted_name_to_snake(p),
            }
        )
